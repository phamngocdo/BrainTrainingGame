...
void GPIO_Init(void) {
    // 1. Enable clocks for GPIOA, GPIOB, SYSCFG, TIM2
    RCC->AHB1ENR |= (0b1 << 0) | (0b1 << 1);     // Enable GPIOA and GPIOB
    ...
    for (int i = 0; i <= 3; i++) {
        ...
        GPIOA->PUPDR &= ~(0b11 << (i * 2));         // No pull-up/down
    }

    // 3. Configure PB0–PB3 as Input with Pull-up + EXTI Interrupt
    for (int i = 0; i <= 3; i++) {
        GPIOB->MODER &= ~(0b11 << (i * 2));         // Input mode
        GPIOB->PUPDR &= ~(0b11 << (i * 2));
        GPIOB->PUPDR |=  (0b01 << (i * 2));         // Pull-up

        // Map EXTI line i to PORTB
        uint8_t exti_index = i / 4;
        uint8_t offset = (i % 4) * 4;
        SYSCFG->EXTICR[exti_index] &= ~(0b1111 << offset); // Clear
        SYSCFG->EXTICR[exti_index] |=  (0b0001 << offset); // 0b0001 = PORTB

        EXTI->IMR  |= (1 << i);   // Unmask EXTI line i
        EXTI->RTSR |= (1 << i);   // Rising edge trigger
    }

    // 4. Enable EXTI interrupts in NVIC (EXTI0–EXTI3 for PB0–PB3)
